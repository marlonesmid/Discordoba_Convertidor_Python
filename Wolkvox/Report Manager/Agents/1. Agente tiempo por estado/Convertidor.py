...
RUTA_BASE       = r"C:\Convertidor\Wolkvox\Report Manager\Agents\1. Agente tiempo por estado"
ARCHIVO_ENTRADA = os.path.join(RUTA_BASE, 'agente_tiempo_estado.json')
ARCHIVO_SALIDA  = os.path.join(RUTA_BASE, 'inserts_agente_tiempo_estado.sql')
TABLA = "QUIRAMA.dbo.Integracion_Wolkvox_ReportManager_Agents_AgenteTiempoPorEstado"

# Campos enteros — se insertan sin comillas
CAMPOS_INT = {'calls', 'inbound', 'outbound', 'internal', 'hits', 'rpc'}

# 'id' es IDENTITY — SQL Server lo genera automáticamente
COLUMNAS_SQL = [
    'agent_id', 'agent_dni', 'agent_name', 'calls', 'inbound', 'outbound',
    'internal', 'ready_time', 'inbound_time', 'outbound_time', 'acw_time',
    'ring_time', 'login_time', 'aht', 'occupancy', 'aux_time', 'hits', 'rpc',
    'aht_outbund', 'aht_inbound', 'login', 'logout', 'updated_at'
]
