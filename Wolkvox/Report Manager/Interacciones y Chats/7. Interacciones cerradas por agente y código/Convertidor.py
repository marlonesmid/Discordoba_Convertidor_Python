...
RUTA_BASE       = r"C:\Convertidor\Wolkvox\Report Manager\Interacciones y Chats\7. Interacciones cerradas por agente y código"
ARCHIVO_ENTRADA = os.path.join(RUTA_BASE, 'Interacciones_cerradas_por_agente_y_codigo.json')
ARCHIVO_SALIDA  = os.path.join(RUTA_BASE, 'Interacciones_cerradas_por_agente_y_codigo.sql')
TABLA = "QUIRAMA.dbo.Integracion_Wolkvox_ReportManager_InteraccionesyChats_InteraccionesCerradasPorAgenteyCodigo"

# 'id' es IDENTITY — SQL Server lo genera automáticamente
COLUMNAS_SQL = [
    'interactions', 'agent_name', 'cod_act', 'description_cod_act', 'updated_at'
]

def limpiar_valor(valor):
    if valor is None or str(valor).strip().upper() == "NULL" or str(valor).strip() == "":
        return "NULL"
    # Corregir encoding latin1 mal interpretado como UTF-8 (ej: "PatiÃ±o" -> "Patiño")
    try:
        v_fixed = str(valor).encode('latin1').decode('utf-8')
    except (UnicodeEncodeError, UnicodeDecodeError):
        v_fixed = str(valor)
    v_safe = v_fixed.replace("'", "''")
    return f"'{v_safe}'"
            ...
            for r in registros:
                ...
                for col in COLUMNAS_SQL:
                    if col == 'updated_at':
                        ...
                        # Insertar como entero, sin comillas
